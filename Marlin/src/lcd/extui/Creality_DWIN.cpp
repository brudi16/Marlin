  ...
  void yield();
    ...
    if(power_off_type_yes ==0)
    {
      ...
      if ((startprogress += 1) > 200)
			{
        #if ENABLED(POWER_LOSS_RECOVERY)
          if(isMediaMounted() && (power_off_commands_count > 0)) {
      ...
      rtscheck.RTS_SndData(getTargetTemp_celsius(H0), NozzlePreheat);
			...
			rtscheck.RTS_SndData(getFlow_percent(E0), Flowrate);
			...
			if (NozzleTempStatus[0] || NozzleTempStatus[2]) //statuse of loadfilement and unloadfinement when temperature is less than
			{
				...
				if (getActualTemp_celsius(H0) > 215 && NozzleTempStatus[0]!=0)
				{
...
SERIAL_ECHOLN(PSTR("BeginSwitch"));
    ...
    case Bedlevel:
      switch(recdat.data[0])
      {
        ...
        {
          setAxisPosition_mm(BED_TRAMMING_Z_HOP, (axis_t)Z);
          setAxisPosition_mm((X_MIN_BED + 30), (axis_t)X);
          setAxisPosition_mm((Y_MIN_BED + 30), (axis_t)Y);
          waitway = 6;
          break;
        }
        ...
        {
          setAxisPosition_mm(BED_TRAMMING_Z_HOP, (axis_t)Z);
          setAxisPosition_mm((X_MAX_BED - 30), (axis_t)X);
          setAxisPosition_mm((Y_MIN_BED + 30), (axis_t)Y);
          waitway = 6;
          break;
        }
        case lowRightData: // Assitant Level , Back Right 4
        {
          setAxisPosition_mm(BED_TRAMMING_Z_HOP, (axis_t)Z);
          setAxisPosition_mm((X_MAX_BED - 30), (axis_t)X);
          setAxisPosition_mm((Y_MAX_BED - 30), (axis_t)Y);
          waitway = 6;
          break;
        }
        ...
        default:
        {
          SERIAL_ECHOPGM("Unsupported Option Selected", recdat.data[0]);
        }
      }
    ...
    case Filement:
      ...
      {
        if (getActualTemp_celsius(H0) < 215 && recdat.data[0] < 5)
        {
          RTS_SndData(220, 0x1020);
          delay_ms(5);
        ...
        switch(recdat.data[0])
          ...
          {
			setAxisPosition_mm((getAxisPosition_mm(E0) + 10), E0);
            setAxisPosition_mm((getAxisPosition_mm(E0) - 90), E0);
            break;
          }
          case 2: // Load filement1
          {
            setAxisPosition_mm((getAxisPosition_mm(E0) + 120), E0);
            break;
          }
    ...
    case LanguageChoice:
      ...
      switch(recdat.data[0])
      {
        ...
        case 2:
          //PID Hotend
		  onStatusChanged("PID Hotend Tuning");
          injectCommands_P(PSTR("M303E0S215C8U\nG28"));
          break;
        case 3:
          //Init EEPROM
          injectCommands_P(PSTR("M502\nM500"));
		  onStatusChanged("Eeprom reset");
          break;
        case 4:
          //BLTouch Reset
          injectCommands_P(PSTR("M280 P0 S60\nG4 500\nM280 P0 S90\nG4 P500"));
		  onStatusChanged("BL Touch reset");
          break;
        case 5:
          //PID Bed
		  onStatusChanged("PID Bed Tuning");
          injectCommands_P(PSTR("M303E-1S60C8"));
		  break;
